---
type: stock
ticker: "6202"
name: "盛群"
industry: "感測/MCU"
created: 2026-07-12
updated: 2026-07-12
review_by: 2026-10-10
tags: [investment/watchlist]
aliases: [盛群]
current_price: 0.0
forward_pe: "待補充"
valuation_rating: "HOLD"
tactical_score: "MA: 0 / Bias: 0"
tactical_action: "尚無數據"
---
# 盛群 (6202)

> [!NOTE]
> 佔位頁面：尚未透過 00_Inbox 收集此股票的研究素材，目前僅依 `[[2_晶片層_感測_MCU]]` 的產業分類納入追蹤（每日價格/技術面、動能分數、財務指標篩選）。投資論點、EPS 預估、供應鏈細節待補充；未來若收到相關研究素材，將以 ingest 流程疊加進本頁面。

## 🎯 投資建議與核心結論 (Summary)
* **核心論點**：[待補充]

---

## 🔗 相關概念與產業連結 (Related Concepts)
* **關聯產業 (Garden)**：`[[2_晶片層_感測_MCU]]`

---

## 📄 原始文件與連結 (Original Documents)
* (尚無)
