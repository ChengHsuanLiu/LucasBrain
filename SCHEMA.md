...
* **`30_Projects/`**：專案報告與投資決策等可交付的完整成果，細分目錄如下：
  ...
  * **`30_Projects/Stock_Reports/`**：個股深入研報與分析。
  * **`30_Projects/Garden_Reports/`**：產業深入研究報告。
  * **`30_Projects/Invest_Timeline/`**：存放投資重點時間軸報告。
...
* **無損疊加**：將新資訊填入對應的 `10_Stocks/` 或 `20_Garden/` 頁面，會丟進 `00_Inbox` 中的資料均已經初步篩選過，不要遺漏太多重點資訊(尤其是專家訪談筆記)，如果不知道要怎麼歸類的資訊，就在個股頁面或產業頁面中加上時序往下填入，不要覆蓋舊的資訊。
