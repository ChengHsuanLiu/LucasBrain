...
* **產業位置（Industry Position）**：結構性、單一值。依照「需求端→晶片層→上游→系統層→應用端」五層供應鏈架構分類，檔名格式為 `{Tier編號}_{Tier名稱}_{子分類名稱}`（例如 `2_晶片層_ASIC客製晶片`、`3_上游_CCL`）。一檔股票通常只屬於一個產業位置，對應個股 frontmatter 的 `industry:` 欄位（單一值，描述核心業務）。
