...
def generate_summary_markdown(as_of_date):
    ...
    lines.append("")
    lines.append("> [!NOTE]")
    lines.append(f"> 資料來源：使用者每日手動取得追蹤中大戶的券商庫存/期貨帳戶截圖，經視覺辨識萃取。大戶身分以化名/暱稱標示（如：高大戶、林大戶、安大戶），非公開資訊，僅供自身投資決策參考。三類 position_type 換算基準不同，請留意：**融資/現股**為直接觀察到的股數與成本；**股票期貨**已按合約規格換算為等值現股股數（一般股票期貨1口=2張=2,000股，小型股票期貨1口=0.1張=100股），cost_price/market_price取自成交均價/即時價位；**估算(%)**（僅有持股比例、無股數與成本資料的大戶）以「總資金 × 持股比例」反推市值，shares/cost_price/cost_basis/unrealized_pnl均為0，僅market_value可信賴，不適合用於加減碼判斷。各大戶回報日期可能不同步（現股/融資帳戶與股票期貨帳戶常非同一天更新），下方共識分析一律採用「每位大戶各自最新一筆」資料，並標示其實際資料日期。最後更新：**{as_of_date}**。")
    lines.append("")
    lines.append("---")
    lines.append("")
